...
const sendPickerButtons = async ({ sock, msg, from, title, duration, thumbnailBuffer }) => {
  const buttons = [
    { buttonId: BUTTON_AUDIO, buttonText: { displayText: 'Audio' }, type: 1 },
    { buttonId: BUTTON_DOCUMENT, buttonText: { displayText: 'Document' }, type: 1 },
    { buttonId: BUTTON_VIDEO, buttonText: { displayText: 'Video' }, type: 1 }
  ];

  const contextInfo = buildContextInfo({
    title,
    body: `Duration: ${duration} • Pick a format`,
    sourceUrl: CHANNEL_URL,
    thumbnailBuffer
  });

  await sock.sendMessage(from, {
    text: `*${title}*\n⏱ Duration: ${duration}\n\nTap one of the buttons below to continue.`,
    footer: 'JAILBREAK-XMD • Choose a download format',
    buttons,
    headerType: 1,
    contextInfo
  }, { quoted: msg });
